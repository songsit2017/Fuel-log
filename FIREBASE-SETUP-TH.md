...
## 4) เปิด Firebase Storage สำหรับรูปภาพ
1. Firebase Console → Storage → Get started
2. เลือก Region เดียวกับ Firestore ถ้าเลือกได้
3. เปิดแท็บ Rules แล้วนำเนื้อหาจากไฟล์ `storage.rules` ไปวาง จากนั้น Publish
4. ตรวจว่า `storageBucket` ใน `firebase-config.js` ถูกคัดลอกมาจาก Firebase Config แล้ว

รูปใบเสร็จและรูปเรือนไมล์จะถูกย่อในแอปก่อนอัปโหลด และสมาชิกที่มีสิทธิ์ในรถคันนั้นเท่านั้นจึงเปิดดูได้

## 5) อัปโหลดขึ้น GitHub
ต้องอัปโหลดไฟล์ใหม่เหล่านี้ด้วย:
- `firebase-config.js`
- `family-sharing.js`
- `firestore.rules` และ `storage.rules` (เก็บเป็นสำเนา ไม่ถูกโหลดโดยหน้าเว็บ)
...
## หมายเหตุ
- รูปใบเสร็จและรูปเรือนไมล์อัปโหลดไป Firebase Storage และดาวน์โหลดมาเก็บในเครื่องของสมาชิกอัตโนมัติ
