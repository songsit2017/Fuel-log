...
class FirestoreSyncRepository(
    ...
    suspend fun sync(uid: String, email: String?, displayName: String?, photoUrl: String? = null): CloudSyncResult = syncMutex.withLock {
        ...
        // Keeps every member's own name/photo fresh on each vehicle they belong to, even when
        // nothing about the vehicle itself changed (the UPLOAD branch above only fires on
        // content diffs, so without this a photo added after someone's first sync/join would
        // never reach Firestore). firestore.rules' selfProfileRefresh lets any member — owner
        // or not — update just their own members.{uid} entry this way.
        for ((id, cloudDoc) in cloudVehicles) {
            val member = cloudDoc.get("members.$uid") as? Map<*, *> ?: continue
            val upToDate = member["photoUrl"] == photoUrl &&
                member["displayName"] == displayName.orEmpty() &&
                member["email"] == email.orEmpty()
            if (!upToDate) {
    ...
    private suspend fun discoverVehicles(uid: String): Map<String, DocumentSnapshot> {
        ...
        try {
            ...
            throw Exception("Sync failed on ownerUid query for uid '$uid': ${e.message}", e)
        }
        try {
            firestore.collection("vehicles").whereArrayContains("memberUids", uid).get().await()
                .documents.forEach { found[it.id] = it }
        } catch (e: Exception) {
            throw Exception("Sync failed on memberUids query for uid '$uid': ${e.message}", e)
        }
        return found
    }
...
private fun parseMaintenance(vehicleId: String, document: DocumentSnapshot, context: Context) = MaintenanceEntity(
    id = document.id,
    vehicleId = vehicleId,
    name = document.getString("name") ?: context.getString(R.string.maintenance_from_cloud_default_name),
